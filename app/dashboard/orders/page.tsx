    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#6B6B6B] tracking-wide uppercase mt-1">Review sales payments and fulfillment states</p>
        </div>
        <button
          onClick={handleExportCSV}
          disabled={filteredData.length === 0}
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-[#F5F5F5] border border-[#E8E8E8] rounded-xl text-xs font-semibold uppercase tracking-wider text-[#111111] hover:bg-[#E8E8E8] disabled:opacity-40 disabled:cursor-not-allowed transition-all"
        >
          <Download size={14} />
          ...
          <table className="w-full border-collapse text-left text-xs text-[#111111]">
            ...
            <tbody className="divide-y divide-[#E8E8E8]">
              {currentItems.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-[#6B6B6B] uppercase tracking-widest text-[10px]">
                    ...
                    >
                      ...
                      <td className="p-4 font-semibold">
                        <div className="flex items-center gap-2">
                          <ShoppingBag size={12} className="text-[#6B6B6B]" />
                          <span>{item.product_name}</span>
                        </div>
                        <div className="text-[10px] text-[#6B6B6B] font-normal mt-0.5">
                      ...
                      {/* Financials */}
                      <td className="p-4">
                        <span className="font-semibold text-[#111111]">LKR {Number(item.total_amount).toLocaleString()}</span>
                      ...
                      <td className="p-4">
                        ...
                        <div className="text-[10px] text-[#6B6B6B] mt-0.5">Phone: {item.customer_phone}</div>
                        <div className="text-[10px] text-[#6B6B6B] mt-0.5 max-w-[200px] truncate" title={item.customer_address}>
                      ...
                      <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                        {isUpdating ? (
                          ...
                          >
                            <option value="pending" className="bg-[#FFFFFF] text-amber-400 font-bold">Pending</option>
                            <option value="verified" className="bg-[#FFFFFF] text-green-400 font-bold">Verified</option>
                            <option value="rejected" className="bg-[#FFFFFF] text-red-400 font-bold">Rejected</option>
                          </select>
                        )}
                      </td>
                      ...
                      <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                        {isUpdating ? (
                          ...
                          >
                            <option value="pending" className="bg-[#FFFFFF] text-amber-400 font-bold">Pending</option>
                            <option value="processing" className="bg-[#FFFFFF] text-cyan-400 font-bold">Processing</option>
                            <option value="shipped" className="bg-[#FFFFFF] text-blue-400 font-bold">Shipped</option>
                            <option value="delivered" className="bg-[#FFFFFF] text-green-400 font-bold">Delivered</option>
                            <option value="cancelled" className="bg-[#FFFFFF] text-red-400 font-bold">Cancelled</option>
                          </select>
                        )}
                      </td>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-end">
          ...
          />
          <div className="relative w-full max-w-lg h-full bg-[#FFFFFF] border-l border-[#E8E8E8] p-8 flex flex-col justify-between overflow-y-auto z-10">
            <div className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-1.5">
                  <span className="block text-[9px] font-bold tracking-widest text-[#6B6B6B] uppercase">Product Information</span>
                  <div className="flex flex-col gap-1 text-xs text-[#111111] bg-white/[0.02] border border-[#E8E8E8] px-4 py-3 rounded-xl">
                    <span className="font-semibold">{selectedOrder.product_name}</span>
                    <span className="text-[#6B6B6B]">Size: {selectedOrder.size || 'N/A'} | Qty: {selectedOrder.quantity || 1}</span>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <span className="block text-[9px] font-bold tracking-widest text-[#6B6B6B] uppercase">Customer Details</span>
                  <div className="flex flex-col gap-2 text-xs text-[#111111] bg-white/[0.02] border border-[#E8E8E8] px-4 py-3 rounded-xl">
                    <div className="flex items-center gap-2"><User size={14} className="text-[#6B6B6B]"/> {selectedOrder.customer_name}</div>
                    <div className="flex items-center gap-2 text-[#6B6B6B] font-mono">WhatsApp: {selectedOrder.customer_phone}</div>
                    <div className="flex gap-2 text-[#6B6B6B]"><MapPin size={14} className="mt-0.5 shrink-0"/> {selectedOrder.customer_address}</div>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <span className="block text-[9px] font-bold tracking-widest text-[#6B6B6B] uppercase">Financials</span>
                  <div className="flex flex-col gap-1 text-xs text-[#111111] bg-white/[0.02] border border-[#E8E8E8] px-4 py-3 rounded-xl">
                    <div className="flex justify-between border-b border-[#E8E8E8] pb-2 mb-2">
                ...
                <div className="space-y-1.5">
                  <span className="block text-[9px] font-bold tracking-widest text-[#6B6B6B] uppercase flex items-center gap-2">
